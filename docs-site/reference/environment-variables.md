...
| `DWARA_OTLP_ENDPOINT` | unset | Base OTLP collector endpoint (`http://` only). Only live in an `otlp`-feature build; reserved-but-inert otherwise. Exports traces to `/v1/traces` and metrics to `/v1/metrics`. |
