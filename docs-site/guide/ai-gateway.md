...
Protocol) server and router. Configured tools are exposed over
JSON-RPC 2.0 on a reserved HTTP path (default `/mcp`); the gateway
authenticates every request, authorizes per-tool access, proxies tool
calls to upstream HTTP endpoints, manages agent sessions in the state
store, and correlates tool calls in analytics.
...
- `dwara_mcp_sessions_total{state}` -- MCP session lifecycle
  transitions (DW-087). `state` is `initialized` (a new session
