...
::: info Status
Response validation compiles into every build — there is no
`openapi_validation` cargo feature (see
[Editions](./editions#scaffolded-surfaces)). The validation engine is
complete and test-covered as a library component. The route-level
config wiring has not landed yet -- the `openapi_validation:` block
below illustrates the target surface and is not in the generated
