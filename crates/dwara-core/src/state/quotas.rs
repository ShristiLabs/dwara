...
// Re-export the pure quota types from config::quotas so existing
// callers (`use crate::state::quotas::{Budget, QuotaOutcome, ...}`)
// continue to work. The canonical definitions now live in
// `config::quotas` (the lowest consuming domain) so both
// `state::quotas` and `extensions::redis_quotas` can share them
// without violating the downward-only dependency direction.
pub use crate::config::quotas::{day_window, month_window, retry_after, Budget, QuotaOutcome};
pub use crate::config::quotas::{DAILY_KEY, MONTHLY_KEY};
...
mod tests {
    ...
    // Local copies of the calendar-arithmetic helpers (now in
    // config::quotas but private there) for deterministic test epochs.
