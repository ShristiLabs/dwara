...
async fn engine_credential_rule_keys_by_consumer_and_rules_are_isolated() {
    ...
    for _ in 0..2 {
        assert!(matches!(
            engine.check(&ctx_consumer("10.0.0.2", None), &[], &per_cred_only, &[]),
            RateLimitOutcome::Allowed { .. }
        ));
    }
    assert!(matches!(
